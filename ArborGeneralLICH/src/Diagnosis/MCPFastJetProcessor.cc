...
MCPFastJetProcessor aMCPFastJetProcessor;

using namespace EVENT;

MCPFastJetProcessor::MCPFastJetProcessor() : Processor("MCPFastJetProcessor")
{
	_description = "Using the FastJet library to identify jets";

	// the input & output collections
	registerInputCollection(LCIO::MCPARTICLE, "recParticleIn", "a list of all reconstructed particles we are searching for jets in.", _lcParticleInName, "MCParticle");
	registerOutputCollection(LCIO::RECONSTRUCTEDPARTICLE, "jetOut", "The identified jets", _lcJetOutName, "JetOut");

	registerOutputCollection(LCIO::MCPARTICLE, "recParticleOut", "a list of all reconstructed particles used to make jets. If no value specified collection is not created", _lcParticleOutName, "");
...
MCPFastJetProcessor::~MCPFastJetProcessor()
{
}
...
void MCPFastJetProcessor::initClusterMode()
{
...
void MCPFastJetProcessor::initJetAlgo()
{
...
bool MCPFastJetProcessor::isJetAlgo(string algo, int nrParams, int supportedModes)
{
...
void MCPFastJetProcessor::initRecoScheme()
{
...
void MCPFastJetProcessor::initStrategy()
{
 ...
 */
void MCPFastJetProcessor::processEvent(LCEvent * evt)
{

	// clear the list containing the (pseudo)-jets and the FJ--to-PFOReconstructedParticle map
	_pjList.clear();
...
//    lccJetsOut->setSubset(true);
	...
	if (_storeParticlesInJets){
	  lccParticlesOut= new IMPL::LCCollectionVec(LCIO::MCPARTICLE);
...
//        rec = getRecPar( (*it), cs.constituents(*it) );
		...
		if (_storeParticlesInJets) 
		{
			...
			{
				MCParticle* p = dynamic_cast< MCParticle* > (_reconstructedPars->getElementAt((cs.constituents(*it))[n].user_index()));
...
vector < fastjet::PseudoJet > MCPFastJetProcessor::doIterativeInclusiveClustering()
{
...
EVENT::ReconstructedParticle* MCPFastJetProcessor::getRecPar(fastjet::PseudoJet& fj, const vector< fastjet::PseudoJet >& constituents )
{
	...
	{
		MCParticle* p = dynamic_cast< MCParticle* > (_reconstructedPars->getElementAt(constituents[n].user_index())) ;
        ReconstructedParticleImpl* new_p = new ReconstructedParticleImpl();
        double new_mom[3] = {p->getMomentum()[0], p->getMomentum()[1], p->getMomentum()[2]};
        new_p->setMomentum( new_mom );
        new_p->setType( p->getPDG() );
        new_p->setEnergy( p->getEnergy() );
		jet->addParticle( new_p );
//        jet->addElement( p );
...
void MCPFastJetProcessor::convertFromRecParticle(LCCollection* recCol)
{
	...
	{
		MCParticle* par = dynamic_cast<MCParticle*> (recCol->getElementAt(i));
//        ReconstructedParticleImpl* new_par = new ReconstructedParticleImpl();
//        new_par->setMomentum(par->getMomentum());
//        new_par->setEnergy(par->getEnergy());
//        ReconstructedParticle* par;
		_pjList.push_back(
