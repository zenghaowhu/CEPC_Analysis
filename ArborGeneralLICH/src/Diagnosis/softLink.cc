...
{		
	...
	if (evtP) 								
	{
        try{
                        ...
                        if(count == 0){
                            ...
                            for(int j = 0; j<num_parent92; j++){    //only used to find out the parton which decay to 92
                                ...
                                if(abs(PDG_parent92) == 1 || abs(PDG_parent92) == 2 || abs(PDG_parent92) == 3 || abs(PDG_parent92) == 4 || abs(PDG_parent92) == 5 || abs(PDG_parent92) == 6){
                                    ...
                                    while(parent92->getParents().size() != 0 && parent92->getPDG() != 94);
                                    if(parent92->getPDG() == 94){
                                        testmass = parent92->getMass();
                                    }
                                }
                            }
                        }
            ...
            int num_94 = no94.size();
            for(int i = 0; i<num_94; i++){
                MCParticle* no941 = no94.at(i);
                MCParticle* parent9411 = no941->getParents()[0];
                MCParticle* parent9412 = no941->getParents()[1];
                if(no941->getMass() == testmass){
                    special += 1;
                    cout<<"special : "<<special<<endl;
                    for(int i = 0; i<4; i++){
                        MCParticle* oriquark = MCTQuark.at(i);
                        if(oriquark->getEnergy() == parent9411->getEnergy() || oriquark->getEnergy() == parent9412->getEnergy()){ quark941.push_back(oriquark);}
                        else {quark942.push_back(oriquark);}
                    }
                }
            }
            ...
            if(special == 0)
            {
                for(int i = 0; i<num_94; i++){
                    ...
                    MCParticle* parent9411 = no941->getParents()[0];
                    MCParticle* parent9412 = no941->getParents()[1];
                    if(no941->getMass() == testmass2){
                        special += 1;
                        cout<<"special222 : "<<special<<endl;
                        for(int i = 0; i<4; i++){
                            MCParticle* oriquark = MCTQuark.at(i);
                            if(oriquark->getEnergy() == parent9411->getEnergy() || oriquark->getEnergy() == parent9412->getEnergy()){ quark942.push_back(oriquark);}
                            else {quark942.push_back(oriquark);}
                        }
                    }
                }
