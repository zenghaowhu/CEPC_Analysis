...
class Hmumu  : public marlin::Processor
{
	public:

		Processor*  newProcessor() { return new Hmumu ; }
...
//		float _CosTheta, _Phi, _Charge;
		...
		int Type, Charge;
    int muon_count_minus, muon_count_plus;
    float energy_muon_first, energy_muon_second;
    float maxminusvalue, maxplusvalue;
		float Energy, TrkSumEn; 
		float P[3], CluEnCom[2];
		float CluEn;
        float Mass_mumu_value;
